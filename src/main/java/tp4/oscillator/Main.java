...
//        }
        ...
        double step = 0.01;
        VelocityVerlet.VerletState state = VelocityVerlet.initialState(amplitude, - gamma / (2.0 * mass));
        for (double t = step; t < 10; t += step) {
            state = VelocityVerlet.nextStep(state, step, (pos, vel) -> (- k * pos - gamma * vel) / mass);
            dumper.print2D(0, state.x);
            dumper.dump(t);
        }
    }
}
